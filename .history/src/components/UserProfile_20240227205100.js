...
export default function UserProfile() {
  ...
  // Function to format numbers, could be used to format the views, likes, and followers
  const formatNumber = (num) => {
    return num.toLocaleString(); // This will format the number with commas (e.g., 12,800)
  };
  ...
  return (
    <main className="max-w-[774px] w-full flex flex-col gap-24 mt-48 p-10 relative">
      ...
      <section className="flex flex-col gap-[51px] px-2.5">
        ...
        <section className="flex flex-col gap-[18px]">
          ...
          <div className="flex items-center gap-4 ">
            {accounts.map((account, index) => (
              <div
                key={index}
                className="flex flex-col items-center gap-2.5 p-2.5 bg-white rounded-md text-black"
              >
                <div className="w-full flex items-center justify-center gap-2.5 text-xs">
                  {/* Replace the src with the appropriate icon based on the platform */}
                  <img
                    src={
                ...
                <div className="w-full flex items-center gap-6">
                  <div className="flex flex-col items-center gap-0.5">
                    <h4 className="text-[8px] font-semibold">
                      {formatNumber(account.views)}k
                    </h4>
                  ...
                  <div className="flex flex-col items-center gap-0.5">
                    <h4 className="text-[8px] font-semibold">
                      {formatNumber(account.followers)}k
                    </h4>
                    <p className="text-[4px] text-[#00000066] text-center">
                      Followers
                    </p>
