  ...
  return (
    <main className="max-w-[774px] w-full flex flex-col gap-24 mt-48 p-10 relative">
      ...
      {accounts.map((account, index) => (
        <div key={index} className="flex flex-col items-center gap-2.5 p-2.5 bg-white rounded-md text-black">
          <div className="w-full flex items-center gap-2.5 text-xs">
            {/* Replace the src with the appropriate icon based on the platform */}
            <img src={account.platform === 'tiktok' ? TikTokIcon : account.platform === 'youtube' ? YouTubeIcon : IG} alt={`${account.platform} icon`} />
          ...
          <div className="w-full flex items-center gap-6">
            <div className="flex flex-col items-center gap-0.5">
              <h4 className="text-[8px] font-semibold">{formatNumber(account.views)}</h4>
              <p className="text-[4px] text-[#00000066] text-center">
                Total lifetime views
              </p>
            </div>
            <div className="flex flex-col items-center gap-0.5">
              <h4 className="text-[8px] font-semibold">{formatNumber(account.likes)}</h4>
              <p className="text-[4px] text-[#00000066] text-center">
                Likes
              </p>
            </div>
            <div className="flex flex-col items-center gap-0.5">
              <h4 className="text-[8px] font-semibold">{formatNumber(account.followers)}</h4>
              <p className="text-[4px] text-[#00000066] text-center">
                Followers
              </p>
            </div>
